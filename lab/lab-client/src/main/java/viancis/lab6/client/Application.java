...
public class Application {
    ...
    private HashMap<String, CommandType> receiveAllCommands() {
        Request request = new Request("commands", null, null,null,null);
    ...
    public void sendAndReceived(ClientInput.UserParseCommand userParseCommand){
        ...
        try {
            ...
            return;
        } catch (RequestValidator.AuthException e) {
            sender.printMessage(new Message(Category.ERROR, "Ошибка auth для " + userParseCommand.command()));
            return;
        }
        try {
            Response response = handlerClient.receiveResponse();
            if (response != null) {
                if (Objects.equals(response.commandName(), "auth")){
                    requestValidator.setToken(response.result());
                }
...
//        try {
//            while (true){
//                Request commandTest = new Request("show",new String[0],null,null, "eyJhbGciOiJIUzI1NiJ9.eyJwYXNzd29yZCI6IjEyMyIsImF1dGhfZGF0ZSI6MTcxNDMyOTA4OTMwNiwidXNlcm5hbWUiOiIxMjMifQ.32gNi4HYhywZ2WS6bG1C3jGVyfl6uREIiUMPG3yY4Fk");
//                ByteArrayOutputStream baos = new ByteArrayOutputStream();
//                ObjectOutputStream oos = new ObjectOutputStream(baos);
//                oos.writeObject(commandTest);
//
//                byte[] commandBytes = baos.toByteArray();
//
//                this.handlerClient.sendData(commandBytes);
